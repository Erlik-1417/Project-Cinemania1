import{d as f,h as b,e as L,b as w,s as $,f as M,j as E,k as v,r as k,l as B,i as S}from"./ui-DrZ8pvwq.js";import{i as I,a as T}from"./hero-D9ul_Bbh.js";const c=document.getElementById("weeklyList"),g=document.getElementById("upcomingWrapper");async function x(){f();try{await Promise.allSettled([H(),A()])}catch(s){console.error(s)}finally{b()}}async function H(){if(!c)return;const n=(await L("week")).results.slice(0,3),o=await Promise.all(n.map(async e=>{const t=e.vote_average||0,d=w(t,"movie-card__star"),l=e.poster_path?`https://image.tmdb.org/t/p/w500${e.poster_path}`:"./img/oops-logo.png",r=e.release_date?e.release_date.slice(0,4):e.first_air_date?e.first_air_date.slice(0,4):"—";return`
    ...
    `}));c.innerHTML=o.join(""),c.querySelectorAll(".movie-card").forEach(e=>{e.addEventListener("click",()=>{$(e.dataset.id)})})}async function A(){if(!g)return;const n=(await M()).results;if(!n||n.length===0)return;const o=Math.floor(Math.random()*Math.min(n.length,10)),e=n[o],t=await E(e.id),d=t.vote_average||0,l=t.poster_path?`https://image.tmdb.org/t/p/w500${t.poster_path}`:"./img/oops-logo.png",r=t.release_date||"—",h=t.genres.map(p=>p.name).slice(0,2).join(", ")||"—";g.innerHTML=`
