import{E as _,F as u,v as b,w as h,y as f,G as $,H as w,I as m,J as L,K as M,z as S,A as k}from"./ui-bYA6f8x1.js";const c=document.getElementById("weeklyList"),g=document.getElementById("upcomingWrapper");async function E(){_();try{await Promise.allSettled([B(),x()])}catch(n){console.error(n)}finally{u()}}async function B(){if(!c)return;const t=(await b("week")).results.slice(0,3),i=await Promise.all(t.map(async a=>{const e=a.vote_average||0,o=h(e,"movie-card__star"),r=a.poster_path?`https://image.tmdb.org/t/p/w500${a.poster_path}`:"./img/oops-logo.png",l=a.release_date?a.release_date.slice(0,4):a.first_air_date?a.first_air_date.slice(0,4):"—";return`
  ...
  `;const s=document.getElementById("upcomingAddBtn"),d=()=>{m(a.id)?(s.textContent="Remove from my library",s.classList.add("upcoming__btn--remove")):(s.textContent="Add to my library",s.classList.remove("upcoming__btn--remove"))};d(),s.addEventListener("click",()=>{m(a.id)?L(a.id):M(a),d()})}async function v(){S(),k(),_();try{await E()}finally{u()}}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",v,{once:!0}):v();
